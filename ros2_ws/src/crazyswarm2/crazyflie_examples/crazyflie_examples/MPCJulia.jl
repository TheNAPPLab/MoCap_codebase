...
function NonlinearMPCController(x_initial::Vector{Float64}, x_ref::Matrix{Float64})
    println("Julia received: x_initial=$x_initial, x_ref size=$(size(x_ref))")
    N=30
    Ts=0.05
    ...
    R = Diagonal([0.1, 0.1, 0.5, 0.1])  # penalize large velocities
    ...
    for n in 1:N
        ...
        # @constraint(model, x[3,n] >= 0.04) # Ensure the drone stays above the ground
        # @constraint(model, x[3,n] >= 0.0)  # remove floor for sim testing
    end
    @constraint(model, x[3, N+1] >= 0.00) # Ensure the final position is also above the ground
    ...
    # NEW — add R_delta above the loop, then add the delta block inside it
    R_delta = Diagonal([0.5, 0.5, 0.3, 0.2])  # add this line before the loop, near where R is defined
    ...
    for k in 1:N
        ...
        # NEW: delta-u penalty — starts at k=2 because k=1 has no previous step to compare
        if k >= 2
            for i in 1:nu
                step_control_cost += R_delta[i,i] * (u[i,k] - u[i,k-1])^2
            end
        end
    ...
    terminal_cost =0.0
    for i in 1:nx
        # NEW — use the last column of the horizon as the terminal reference
        terminal_cost += terminal_weight * Q[i,i] * (x_ref[i, N+1] - x[i,N+1])^2
...
function TestFunction()
    ...
    x_target = [0.0, 0.0, 0.5, 0.0]


    x_ref = referencetrajectory(x_initial, x_target, N, Ts)
    x_actual, u_actual = NonlinearMPCController(x_initial, x_ref)
